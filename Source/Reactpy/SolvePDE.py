    ...
    def __init__(self, grid_matrix: list, Delta: float):
        ...
        for row_index in range(self.size[1]):
          for column_index in range(self.size[0]):
              if isinstance(grid_matrix[row_index][column_index], float) or isinstance(grid_matrix[row_index][column_index], int):
                  self.cells_list.append(Cell(grid_matrix[row_index][column_index], (row_index, column_index), "Flux"))
                  ...
                  raise ValueError("Cell (" + str(row_index) + ", " + str(column_index) + ") type not valid")
    ...
    * `solve()` :
    ...
    def __init__(self, grid: Grid, sources: np.ndarray, PDE_matrix: np.ndarray):
        ...
        `PDE_matrix` : np.array
            2D array that represents the discretized stationary PDE to be solved.
        `sources` : np.array
            2D array that contains the source terms of the PDE: these are arranged in the array following the 
            geometry of the reactor.

        See also
        --------
        `Grid.first_Xderivative_matrix()`, `Grid.first_Yderivative_matrix()`, `Grid.second_Xderivative_matrix()` 
    ...
    def solve(self, omega: float, conv_criterion: float, update: bool = False) -> Grid:
        ...
        1. first, the PDE_matrix is decomposed into its lower tringular component $L$ and the upper one $-U$
        2. iterating, from an initial guess,  $$\vec \phi_{n}= L^{-1}U \vec \phi_{n-1}+L^{-1} \vec S,$$ where $\vec S$ is `sources`, we obtain approximations of the solution
        ...
        conv_criterion : float
            Criterion of convergence: if the norm of two consecuitive solutions (vectors) is smaller that this parameter convergence is met.
        update : bool
            If True the attribute `grid` is updated with the solution, otherwise only the retuned Grid object contains the solution.
